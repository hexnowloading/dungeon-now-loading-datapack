...
execute as @e[type=marker, tag=dnl.monolith, distance=..1.1] if score @s dnl.lid = #dnl.monolith.lid dnl.lid run function dnl:util/monolith/honey_potion/ingredient_6/send_data
execute align xyz positioned ~-1 ~ ~-1 as @e[type=item_frame, tag=dnl.monolith.ingredient, tag=dnl.potion, tag=!dnl.monolith.used, dx=2, dy=2, dz=2, limit=1, sort=random] run function dnl:util/monolith/ingredient_used
